...
plots <- read.csv("simulation_data/linear_trend_plots.csv")
info <- read.csv("simulation_data/linear_trend_info.csv")
scagnostics <- read.csv("simulation_data/linear_wide_scagnostics.csv")
...
```
IDEAS:
-Methods to try: model probabilities, medians, global mean, leave one out
-Leave One Out
-->Compare mean of 19 with 1 left out for all 20 plots
-Possibly aggregate with multiple models/distance functions (wacky idea)
-Run 1000ish trials and compute accuracy for each prediction type
-Try more complicated distributions (Time Series!, residuals)
-->Maybe look at combined data 
...
accuracy
#EU - .581, MA - .857, QDA - .912, LOG - .936, LEU - .581, $LMA - 85.7
```
NOTES:
...
QDA Regression: 91.2% Accuracy
Logistic Regression: 93.6% Accuracy
Euclidean Distance: 58.1% Accuracy
Mahalanobian Distance: 85.7% Accuracy
LOO Euclidean Distance: 58.1% Accuracy
LOO Mahalanobian Distance: 85.7% Accuracy

Results:
Leave-One-Out method does not seem to produce different results from using all 20 plots. 
Supervised methods are much more accurate than unsupervised
Euclidean distance is about as good as random guessing
