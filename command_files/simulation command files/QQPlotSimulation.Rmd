...
for(i in 1:100){
  for(n in Ns) {
    for(df in DF){
      #normal
      for(i in 1:4){ 
        norm <- qqnorm(rnorm(n, 0,sqrt(df/(df-2))), plot.it = F)
      ...
      #logNormal
      logNorm <- qqnorm(rlnorm(n, sdlog = sqrt(log(0.5 + 0.5*sqrt(1 + 4*df/(df-2))))), plot.it = F)
      ...
      #Chi-Squared
      chisq <- qqnorm(rchisq(n, df), plot.it = F)
...
for (plot in QQPlots) {
  ...
  QQscagnostics[[index]] <- list(ID = index, scagnostics = scagnostics(plot$x, plot$y)$s)
  index = index + 1
}
```
