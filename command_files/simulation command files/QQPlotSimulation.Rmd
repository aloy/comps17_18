...
#Additional scagnostic-type measures
```{r}
deviation <- function(qqplot){
  ...
  for(n in Ns) {
    for(df in DF){
      ...
      for(i in 1:4){ 
        ...
        QQPlots[[index]] <- list(ID = index,  x = norm$x , y = norm$y, origdata = normdata )
      ...
      QQPlots[[index]] <- list(ID = index,  x = t$x , y = t$y, origdata = tdata)
      ...
      QQPlots[[index]] <- list(ID = index,  x = exp$x , y = exp$y, origdata = expdata)
      ...
      QQPlots[[index]] <- list(ID = index,  x = logNorm$x , y = logNorm$y, origdata = logNormData)
      ...
      QQPlots[[index]] <- list(ID = index,  x = chisq$x , y = chisq$y, origdata = chisqData)
...
for (plot in QQPlots) {
  ...
  QQscagnostics[[index]] <- list(ID = index, scagnostics = scagnostics(plot$x, plot$y)$s, AD_pvalue = ad.test(plot$origdata)$p.value, SW_pvalue = shapiro.test(plot$origdata)$p.value, CVM_pvalue = cvm.test(plot$origdata)$p.value, lillie_pvalue = lillie.test(plot$origdata)$p.value, pearson_pvalue = pearson.test(plot$origdata)$p.value, SF_pvalue = sf.test(plot$origdata)$p.value)
  index = index + 1
}
```
...
save the CSV
```{r}
write.csv(info_df, "QQPlots_info.csv")
write.csv(QQPlots_df, "QQplots.csv")
write.csv(QQscagnostics_df, "QQPlots_scagnostics.csv")
```
