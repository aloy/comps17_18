...
convex.low <- ggplot(ex1, aes(x = V1, y = V2)) +
  geom_point() +
  geom_polygon(data = ex1[path_nodes, ], aes(color = "raspberry"),  alpha = 0.5, fill = NA) +
  geom_segment(aes(color = "raspberry"), x = .002643, xend = .931209 , y = .004299, yend = .0043299) +
  geom_segment(aes(color = "raspberry"), x = .002643, xend = .002643 , y = .004299, yend = 1.114296) +
  geom_segment(aes(color = "navyblue"), x = .002643, xend = .931209, y = 1.114296, yend = .004299) +
  geom_polygon(aes(color = "navyblue"), data = filter(df.hull1, !is.na(hull)), alpha = 0.5, fill = NA) +
  geom_segment(aes(color = "navyblue"), x = .002643, xend = .931209 , y = .004299, yend = .004299) +
  theme(legend.position = "none", axis.text = element_blank(), axis.ticks = element_blank()) +
  labs(x = "", y = "")
convex.low
ggsave("for_presentation/convexPlotLow_Hulls.png", width = 8, height = 6, dpi = 500)
...
convex.med <- ggplot(df.main2, aes(x = V1, y = V2)) +
  geom_point() +
  geom_polygon(data = ex2[path_nodes, ], aes(color = "raspberry"),  alpha = 0.5, fill = NA) +
  geom_polygon(aes(color = "navyblue"), data = filter(df.hull2, !is.na(hull)), alpha = 0.5, fill = NA) +
  theme(legend.position = "none", axis.text = element_blank(), axis.ticks = element_blank()) +
  labs(x = "", y = "")
convex.med
ggsave("for_presentation/convexPlotMed_Hulls.png", width = 8, height = 6, dpi = 500)
...
convex.high <- ggplot(ex3, aes(x = V1, y = V2)) +
  geom_point() +
  geom_polygon(data = ex[path_nodes, ], aes(color = "raspberry"),  alpha = 0.5, fill = NA) +
  geom_polygon(aes(color = "#257985"), data = filter(df.hull3, !is.na(hull)), alpha = 0.5, fill = NA) +
  theme(legend.position = "none", axis.text = element_blank(), axis.ticks = element_blank()) +
  labs(x = "", y = "")
convex.high
ggsave("for_presentation/convexPlotHigh_Hulls.png", width = 8, height = 6, dpi = 500)
...
eu <- mvrnorm(500, mu = c(2,2), Sigma = sigma1)
set.seed(100)
sigma2 <- matrix(c(1, 0.9, 0.9, 1), 2) 
mah <- mvrnorm(500, mu = c(2,2), Sigma = sigma2)

plot(eu[,1], eu[,2])
plot(mah[,1], mah[,2])

eu2 <- data.frame(x = eu[,1], y = eu[,2])
mah2 <- data.frame(x = mah[,1], y = mah[,2])



ggplot(eu2) + 
  geom_point(aes(x = x, y = y)) + 
  geom_point(aes(x = 0, y = 5), color = "red", size = 3) + 
  geom_point(aes(x = 4, y = 5), color = "blue", size = 3) + 
