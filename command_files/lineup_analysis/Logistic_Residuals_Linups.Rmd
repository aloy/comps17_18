---
title: "Logistic Residuals Linups"
author: "Logan Crowl"
date: "3/4/2018"
...
plots <- read_csv("simulation_data/logistic_residuals_plots.csv")
info <- read_csv("simulation_data/logistic_residuals_info.csv")
scagnostics <- read_csv("simulation_data/logistic_residuals_scagnostics.csv")
plots <- plots[,-1]
info <- info[,-1]
...
for(i in 1:18){
print(which.max(lof(lineup_scagnostics[, 2:10], k = 2:19)[,i]))
}

lof <- numeric(18)
for(i in 1:18){
lof[i] <- which.max(lof(lineup_scagnostics[, 2:10], k = 2:19)[,i])
}

Mode(lof)

#Let's do a large scale test
...
for(i in 1:10){
  ...
  lof <- numeric(18)
  for(j in 1:18){
  lof[j] <- which.max(lof(lineup_scagnostics[, 2:10], k = 2:19)[,j])
  }
...
for(i in 1:100){
  ...
  null_scagnostics <- sample_n(null, 19)
  lineup_scagnostics <- rbind(signal_scagnostics, null_scagnostics)
  prediction[i] <- which.max(rowMeans(lof(lineup_scagnostics[, 2:10], k = 2:19)))
  print(i)
}

mean(prediction == 1)

#accuracy of 0.26 seems pretty bad.  Maybe let's not use LOF for log residuals
