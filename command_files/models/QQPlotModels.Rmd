...
#Logistic Model
```{r}
QQ.glm <- glm(as.factor(signal) ~ scag_num_1+scag_num_2+scag_num_3+scag_num_4+scag_num_5+scag_num_6+scag_num_7+scag_num_8+scag_num_9 + N, data = scags, family = "binomial")
...
#cross-validation
predictors <- c("scag_num_1", "scag_num_2", "scag_num_3", "scag_num_4", "scag_num_5", "scag_num_6", "scag_num_7", "scag_num_8", "scag_num_9")
#not working :(
cv.rf1 <- rfcv(scags[,predictors], as.factor(scags$signal), cv.fold = 5)
cv.rf1$error.cv


#Tune the model

QQ.rf2 <- tune.randomForest()
```

Cross validated error rate of 20.9%
